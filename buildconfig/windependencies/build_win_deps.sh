...
# copy in libwinpthread
cp $MINGW_PREFIX/bin/libwinpthread-1.dll $PG_DEP_PREFIX/bin/libwinpthread-1.dll

# Download and setup strings.h
curl -sL --retry 10 https://raw.githubusercontent.com/win32ports/strings_h/refs/heads/master/strings.h > $PG_DEP_PREFIX/include/strings.h

# Make MSVC-compatible .lib files from MinGW .dll files
cd $PG_DEP_PREFIX
for lib in SDL2 SDL2_image SDL2_ttf SDL2_mixer libfreetype-6 libportmidi; do
    cd bin
    gendef -a "${lib}.dll"
    cd ..
    dlltool -d "bin/${lib}.def" -D "bin/${lib}.dll" -l "lib/${lib}.lib"
done
mv lib/libfreetype-6.lib lib/freetype.lib
mv lib/libportmidi.lib lib/portmidi.lib

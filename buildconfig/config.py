...
def writesetupfile(deps, basepath, additional_lines):
    ...
    with open(sdl_setup_filename) as origsetup, open(
...
Only SDL2 is supported now."""
    ...
    if sys.platform == "win32":
        additional_platform_setup = open(
            os.path.join(BASE_PATH, "buildconfig", "Setup_Win_Camera.in")
        ).readlines()
    elif sys.platform == "darwin":
        additional_platform_setup = open(
        ...
        additional_platform_setup = open(
            os.path.join(BASE_PATH, "buildconfig", "Setup.Emscripten.SDL2.in")
        ).readlines()
    else:
        additional_platform_setup = open(

...
def spawn(self, cmd, **kwargs):
    ...
    if (
        machine_name.startswith(("x86", "i686")) or machine_name.lower() == "amd64"
...
if IS_PYPY and sys.pypy_version_info < (7,):
    raise SystemExit(
        "Pygame-ce requires PyPy version 7.0.0 above, compatible with CPython >= 3.8"
    )
...
if compile_cython:
    ...
    pyx_files = glob.glob(
        os.path.join("src_c", "cython", "pygame", "*.pyx")
...
# examples
add_datafiles(
    data_files, "pygame/examples", ["examples", ["README.rst", ["data", ["*"]]]]
)

# docs
add_datafiles(
    data_files,
    "pygame/docs/generated",
    [
        "docs/generated",
        [
            ...
            [
                ...
                ["*.html", "*.js", "*.json"],  # Reference pages  # Comments script
            ],  # Comment data
            ["c_api", ["*.html"]],  # pygame C API
            ["tut", ["*.html", ["tom", ["*.html", "*.png"]]]],  # Tutorials
            [
                "_static",  # Sphinx added support files
                ["*.css", "*.png", "*.ico", "*.js", "*.zip", "*.svg"],
            ],
            [
                "_images",  # Sphinx added reST ".. image::" refs
                ["*.jpg", "*.png", "*.gif"],
            ],
            ["_sources", ["*.txt", ["ref", ["*.txt"]]]],  # Used for ref search
        ],
    ],
)
...
class LintFormatCommand(Command):
    ...
    def run(self):
        ...
        c_files = filter_files(
            path_obj, c_files_unfiltered, c_file_allow, c_file_disallow
        )
        ...
        for linter, option in commands.items():
            ...
            if result.returncode:
                msg = f"'{linter}' failed."
                msg += (
                    " Please run: python setup.py format"
                    if linter in formatters
                    else ""
                )
...
class DocsCommand(Command):
    ...
    def run(self):
        ...
        command_line = [sys.executable, os.path.join("buildconfig", "makeref.py")]
...
class StubcheckCommand(Command):
    ...
    def run(self):
        ...
        if shutil.which("mypy") is None:
            warnings.warn(
                "Please install 'mypy' in your environment. (hint: 'python3 -m pip install mypy')"
            )
            sys.exit(1)
...
if STRIPPED:
    ...
    PACKAGEDATA = {
        "cmdclass": cmdclass,
        "packages": ["pygame", "pygame._sdl2"],
        "package_dir": {"pygame": "src_py", "pygame._sdl2": "src_py/_sdl2"},
        "ext_modules": extensions,
        "zip_safe": False,
        "data_files": data_files,
    }

...
class SpriteCollideTest(unittest.TestCase):
    ...
    def test_pointcollide__without_collided_callback(self):
        ...
        # test no kill
        expected_list = []
        crashed = sprite.pointcollide(miss_point, self.ag, False)
        self.assertListEqual(expected_list, crashed)

        expected_list = [self.s1]
        crashed = sprite.pointcollide(hit_point, self.ag, False)
        self.assertListEqual(expected_list, crashed)
        ...
        crashed = sprite.pointcollide(hit_point, self.ag2, False)
        crashed = sorted(crashed, key=id)
        self.assertListEqual(expected_list, crashed)

        # Test dokill=True (kill colliding sprites).
        expected_list = [self.s1]
        crashed = sprite.pointcollide(hit_point, self.ag, True)
        self.assertListEqual(expected_list, crashed)
    ...
    def test_pointcollide__with_collided_callback(self):
        ...
        expected_list = [self.s1]
        crashed = sprite.pointcollide(hit_point, self.ag, False, collided_callback_true)
        self.assertListEqual(expected_list, crashed)

        # Test dokill=True (kill colliding sprites).
        expected_list = []
        crashed = sprite.pointcollide(hit_point, self.ag, True, collided_callback_false)

        self.assertListEqual(expected_list, crashed)

        expected_list = [self.s1]
        crashed = sprite.pointcollide(hit_point, self.ag, True, collided_callback_true)
        self.assertListEqual(expected_list, crashed)
        self.assertListEqual(self.ag.sprites(), [])

    def test_pointcollide_mask__opaque(self):
        # make fully opaque sprite that will collide with masks.
        self.s1.image.fill((255, 255, 255, 255))

        hit_point = [0, 0]

        # mask should be autogenerated from image if it doesn't exist.
        self.assertEqual(
            sprite.pointcollide(hit_point, self.ag, False, sprite.pointcollide_mask),
            [self.s1],
        )
    ...
    def test_pointcollide_mask__transparent(self):
        # make fully transparent sprite that won't collide with masks.
        self.s1.image.fill((255, 255, 255, 0))

...
instantianting :func:`pygame.event.Event()` themselves, usually paired
...
::
    ...
    MOUSEMOTION                pos, rel, buttons, touch(1)
    MOUSEBUTTONUP              pos, button, touch(1), clicks(2)
    MOUSEBUTTONDOWN            pos, button, touch(1), clicks(2)
    MOUSEWHEEL                 which, flipped, x, y, touch(1), precise_x,
                                precise_y
    ...
    TEXTEDITING(3)             text, start(3), length(3)
    TEXTINPUT(3)               text

    DROPFILE                   file (Path of the file dropped on the window)
    DROPBEGIN
    DROPCOMPLETE
    DROPTEXT                   (Only supported on X11)
...
(``1``): The ``touch`` attribute of ``MOUSE`` events indicates whether or not the
...
(``2``): The ``clicks`` attribute of the ``MOUSEBUTTONDOWN`` and ``MOUSEBUTTONUP``
...
(``3``): The ``start`` attribute of the ``TEXTEDITING`` event is the cursor position,
...
.. versionadded:: 2.1.3 Adroid events, ``precise_x`` and ``precise_y`` to ``MOUSEWHEEL`` events,

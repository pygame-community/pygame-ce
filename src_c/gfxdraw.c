...
_gfx_pixelcolor(PyObject *self, PyObject *args)
{
    ...
    if (!pgSurface_Check(surface)) {
        return RAISERETURN(PyExc_TypeError, "surface must be a Surface", NULL);
    }
...
_gfx_rectanglecolor(PyObject *self, PyObject *args)
{
    ...
    if (sdlrect == NULL) {
        return RAISERETURN(PyExc_TypeError, "invalid rect style argument",
                           NULL);
    }
...
_gfx_polygoncolor(PyObject *self, PyObject *args)
{
    ...
    if (!PySequence_Check(points)) {
        return RAISERETURN(PyExc_TypeError, "points must be a sequence", NULL);
    }

    count = PySequence_Size(points);
    if (count < 3) {
        return RAISERETURN(PyExc_ValueError,
                           "points must contain more than 2 points", NULL);
    }
...
_gfx_texturedpolygon(PyObject *self, PyObject *args)
{
    ...
    if (!pgSurface_Check(texture)) {
        return RAISERETURN(PyExc_TypeError, "texture must be a Surface", NULL);
    }
...
_gfx_beziercolor(PyObject *self, PyObject *args)
{
    ...
    if (steps < 2) {
        return RAISERETURN(PyExc_ValueError,
                           "steps parameter must be greater than 1", NULL);
    }
    ...
    if (!vx || !vy) {
        ...
        return RAISERETURN(PyExc_MemoryError, "memory allocation failed",
                           NULL);
    }

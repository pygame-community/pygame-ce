...
texture_renderer_draw(pgTextureObject *self, PyObject *area, PyObject *dest)
{
    ...
    if (SDL_RenderCopyExF(self->renderer->renderer, self->texture, srcrectptr, dstrectptr, 0, NULL, SDL_FLIP_NONE) < 0) {
        RAISE(pgExc_SDLError, SDL_GetError());
    }
}
...
texture_draw(pgTextureObject *self, PyObject *args, PyObject *kwargs) {
    ...
    if (!Py_IsNone(dstrectobj)) {
        if (!(dstrectptr = pgFRect_FromObject(dstrectobj, &dstrect))) {
            if (!pg_TwoFloatsFromObj(dstrectobj, &dstrect.x, &dstrect.y)) {
                return RAISE(PyExc_ValueError, "dstrect must be a point, Rect, or None");
            }
            dstrect.w = (float)self->width;
            dstrect.h = (float)self->height;
        }
    ...
    RENDERER_ERROR_CHECK(SDL_RenderCopyExF(self->renderer->renderer, self->texture, srcrectptr, dstrectptr, angle, originptr, flip))
    Py_RETURN_NONE;
}
...
texture_draw_triangle(pgTextureObject *self, PyObject *args, PyObject *kwargs) {
    ...
    if (!Py_IsNone(p1_uvobj) && !pg_TwoFloatsFromObj(p1_uvobj, &p1_uv[0], &p1_uv[1])) {
        return RAISE(PyExc_TypeError, "invalid p1_uv argument");
    }
    if (!Py_IsNone(p2_uvobj) && !pg_TwoFloatsFromObj(p2_uvobj, &p2_uv[0], &p2_uv[1])) {
        return RAISE(PyExc_TypeError, "invalid p2_uv argument");
    }
    if (!Py_IsNone(p3_uvobj) && !pg_TwoFloatsFromObj(p3_uvobj, &p3_uv[0], &p3_uv[1])) {
    ...
    RENDERER_ERROR_CHECK(SDL_GetTextureColorMod(self->texture, &_r_mod, &_g_mod, &_b_mod));
    ...
    RENDERER_ERROR_CHECK(SDL_RenderGeometry(self->renderer->renderer, self->texture, vertices, 3, NULL, 0))
    Py_RETURN_NONE;
#else
...
texture_draw_quad(pgTextureObject *self, PyObject *args, PyObject *kwargs) {
    ...
    if (!Py_IsNone(p4_uvobj) && !pg_TwoFloatsFromObj(p3_uvobj, &p4_uv[0], &p4_uv[1])) {
    ...
    RENDERER_ERROR_CHECK(SDL_RenderGeometry(self->renderer->renderer, self->texture, vertices, 6, NULL, 0))
    Py_RETURN_NONE;
#else
    RAISE(PyExc_TypeError, "draw_triangle() requires SDL 2.0.18 or newer");
    Py_RETURN_NONE;
...
texture_update(pgTextureObject *self, PyObject *args, PyObject *kwargs) {
    ...
    RENDERER_ERROR_CHECK(SDL_QueryTexture(self->texture, &format, NULL, NULL, NULL))
...
texture_set_alpha(pgTextureObject *self, PyObject *arg, void *closure) {
    if (PyLong_Check(arg)) {
        ...
        RENDERER_PROPERTY_ERROR_CHECK(SDL_SetTextureAlphaMod(self->texture, (Uint8)longval))
        return 0;
    }
    return -1;
}
...
texture_set_blend_mode(pgTextureObject *self, PyObject *arg, void *closure) {
    if (PyLong_Check(arg)) {
        ...
        RENDERER_PROPERTY_ERROR_CHECK(SDL_SetTextureBlendMode(self->texture, (int)longval))
        return 0;
    }
    return 1;
}
...
texture_get_color(pgTextureObject *self, void *closure) {
    Uint8 color[4];
    RENDERER_ERROR_CHECK(SDL_GetTextureColorMod(self->texture, &color[0], &color[1], &color[2]));
    color[3] = 255;
...
texture_set_color(pgTextureObject *self, PyObject *arg, void *closure) {
    ...
    RENDERER_PROPERTY_ERROR_CHECK(SDL_SetTextureColorMod(self->texture, color[0], color[1], color[2]))
    return 0;
}

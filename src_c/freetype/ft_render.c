                      ...
                      Angle_t rotation)
{
    ...
    if (style == FT_STYLE_DEFAULT) {
        ...
        if (_PGFT_CheckStyle((FT_UInt32)style)) {
            return RAISERETURN(PyExc_ValueError, "Invalid style value", -1);
        }
    ...
    if ((mode->style & FT_STYLES_SCALABLE_ONLY) && !fontobj->is_scalable) {
        return RAISERETURN(PyExc_ValueError,
                           "Unsupported style(s) for a bitmap font", -1);
    }
    ...
    if (mode->rotation_angle != 0) {
        ...
        if (mode->style & FT_STYLE_UNDERLINE) {
            return RAISERETURN(
                PyExc_ValueError,
                "the underline style is unsupported for rotated text", -1);
        }
    ...
    if (mode->render_flags & FT_RFLAG_VERTICAL) {
        if (mode->style & FT_STYLE_UNDERLINE) {
            return RAISERETURN(
                PyExc_ValueError,
                "the underline style is unsupported for vertical text", -1);
        }
    }
                             ...
                             SDL_Rect *r)
{
    ...
    if (!PG_SURF_BytesPerPixel(surface)) {
        ...
        return RAISERETURN(pgExc_SDLError, "Got surface of invalid format",
                           -1);
    }

...
draw_round_rect(SDL_Surface *surf, int x1, int y1, int x2, int y2, int radius,
                ...
                int bottom_left, int bottom_right, int *drawn_area);
static int
draw_bezier_color(SDL_Surface *dst, const int *vx, const int *vy, int n, int s,
                  Uint32 color);
...
bezier(PyObject *self, PyObject *args, PyObject *kwargs)
{
    ...
    if (!PyArg_ParseTupleAndKeywords(args, kwargs, "O!OiO", keywords,
                                     &pgSurface_Type, &surface, &points,
                                     &steps, &colorobj))
        return NULL;
...
static PyMethodDef _draw_methods[] = {
    ...
    {"bezier", (PyCFunction)bezier, METH_VARARGS | METH_KEYWORDS,
     DOC_DRAW_BEZIER},

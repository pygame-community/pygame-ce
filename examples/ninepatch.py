...
def ninepatch_scale(
    ...
    if dest_surface is not None:
        ...
        ret_surface = pygame.Surface(size, pygame.SRCALPHA*alpha) # when alpha is False the flags become 0
    ...
    if corner_size < 0 or corner_size > min(ret_w, ret_h):
        raise ValueError("Corner size must be nonnegative and not greater than the smaller between width and height")
    ...
    ret_surface.blit( # topright corner
        subsurface(src_w-c, 0, c, c),
        (ret_w-c, 0))
    ret_surface.blit( # bottomleft corner
        subsurface(0, src_h-c, c, c),
        (0, ret_h-c))
    ret_surface.blit( # bottomright corner
        subsurface(src_w-c, src_h-c, c, c),
        (ret_w-c, ret_h-c))
    
    if src_x_side > 0:
        ret_surface.blit( # top side
            scale_func(
                subsurface(c, 0, src_x_side, c), 
                (ret_x_side, c)), 
            (c, 0))
        ret_surface.blit( # bottom side
            scale_func(
                subsurface(c, src_h-c, src_x_side, c),
                (ret_x_side, c)),
            (c, ret_h-c))
        
    if src_y_side > 0:
        ret_surface.blit( # left side
            scale_func(subsurface(0, c, c, src_y_side), 
                       (c, ret_y_side)), 
            (0, c))
        ret_surface.blit( # right side
            scale_func(subsurface(src_w-c, c, c, src_y_side), 
                       (c, ret_y_side)), 
            (ret_w-c, c))
    ...
    if src_x_side > 0 and src_y_side > 0:
        ret_surface.blit( # central area
            scale_func(
                subsurface(c, c, src_x_side, src_y_side),
                (ret_x_side, ret_y_side)), 
            (c, c))
...
def main():
    ...
    pygame.draw.rect(original_surface, "green", original_surface.get_rect(), 0, CORNER_SIZE)
    ...
    ninepatch = ninepatch_scale(original_surface, SCALE_SIZE, CORNER_SIZE, True, False, None)
